 ...
 * @access private
 */
function _elgg_get_view_service() {
    static $service;
    
    if (!isset($service)) {
        $service = new ElggViewService();
    }
...
function elgg_set_viewtype($viewtype = "") {
    _elgg_get_view_service()->setViewtype($viewtype);
    return true;
}
...
function elgg_get_viewtype() {
    return _elgg_get_view_service()->getViewtype();
}
...
function elgg_view_exists($view, $viewtype = '', $recurse = true) {
    return _elgg_get_view_service()->viewExists($view, $viewtype, $recurse);
}
 ...
 */
function elgg_view($view, array $vars = array(), $bypass = false, $debug = false, $viewtype = '') {
    return _elgg_get_view_service()->view($view, $vars, $bypass, $viewtype);
}
